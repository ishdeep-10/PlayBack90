"""Apply the active xG model to existing event parquet files in R2.
...
Examples:
  apps/api/.venv/bin/python -u Data/backfill_xg_to_r2.py --league premier-league --season 2025_2026 --limit 1
  apps/api/.venv/bin/python -u Data/backfill_xg_to_r2.py --league premier-league --season 2025_2026 --limit 1 --apply
  apps/api/.venv/bin/python -u Data/backfill_xg_to_r2.py --league-season premier-league:2025_2026 --league-season laliga:2025_2026 --per-league-limit 5 --apply
  apps/api/.venv/bin/python -u Data/backfill_xg_to_r2.py --league-season premier-league:2025_2026 --league-season laliga:2025_2026 --batch-size 100 --batch-index 0 --skip-complete --apply
"""
...
from app.services.xg_model import apply_shot_xg, predict_shot_xg  # noqa: E402
from Data.xg_features import own_goal_mask, shot_mask  # noqa: E402


DEFAULT_OUTPUT_DIR = ROOT / "models" / "xg" / "v2" / "backfills"
...
def _backup_key(source_key: str, run_id: str) -> str:
    ...
    return f"{settings.r2_bucket}/backups/xg/{run_id}/{source_without_bucket}"
...
def _summarize_file(df: pd.DataFrame, updated: pd.DataFrame, key: str, version: str) -> dict[str, object]:
    ...
    old_xg = pd.to_numeric(df.get("xG", pd.Series(np.nan, index=df.index)), errors="coerce")
    new_xg = pd.to_numeric(updated.get("xG", pd.Series(np.nan, index=updated.index)), errors="coerce")
    changed = ~(old_xg.fillna(-999999).round(8).eq(new_xg.fillna(-999999).round(8)))
    current_version = (
        updated.get("xg_model_version", pd.Series("", index=updated.index)).fillna("").astype(str)
        if "xg_model_version" in updated.columns
    ...
    prediction_rows = predict_shot_xg(df, version=version)
    return {
        "file": key,
        "match_id": match_id,
        ...
        "old_xg": round(float(old_xg[eligible].fillna(0).sum()), 3),
        "new_xg": round(float(new_xg[eligible].fillna(0).sum()), 3),
        "delta_xg": round(float(new_xg[eligible].fillna(0).sum() - old_xg[eligible].fillna(0).sum()), 3),
        "changed_rows": int(changed.sum()),
        "versioned_rows": int(current_version.eq(version).sum()),
        "new_xg_missing": int(new_xg[eligible].isna().sum()),
    }
...
def _is_complete(summary: dict[str, object], version: str) -> bool:
    ...
    return (
        int(summary.get("new_xg_missing", 0)) == 0
...
def backfill(
    ...
    if not files:
        raise RuntimeError("No parquet files found for xG backfill.")
    ...
    for idx, key in enumerate(files, start=1):
        ...
        updated = apply_shot_xg(df, version=version, force=force)
        ...
        if skip_complete and _is_complete(summary, version):
            ...
            print(
                ...
                f"changed_rows={summary['changed_rows']} missing_new={summary['new_xg_missing']} "
    ...
    report_json = output_dir / f"xg_backfill_{version}{label}_summary.json"
    report_csv = output_dir / f"xg_backfill_{version}{label}_matches.csv"
    report_json.write_text(json.dumps(summaries, indent=2), encoding="utf-8")
    pd.DataFrame(summaries).to_csv(report_csv, index=False)

    total_old = sum(float(row.get("old_xg", 0)) for row in summaries)
    total_new = sum(float(row.get("new_xg", 0)) for row in summaries)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backfill v2 xG values into R2 event parquet files.")
    ...
    parser.add_argument("--per-league-limit", type=int, help="Files to process per --league-season bucket.")
    parser.add_argument("--batch-size", type=int, help="Global batch size after file discovery.")
    parser.add_argument("--batch-index", type=int, default=0, help="0-based batch index used with --batch-size.")
    ...
    parser.add_argument("--run-label", help="Run label for local reports and R2 backup path.")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--apply", action="store_true", help="Actually upload updated parquet files to R2.")
    parser.add_argument("--force", action="store_true", help="Recompute rows even if already marked with this xG version.")
    parser.add_argument("--skip-complete", action="store_true", help="Skip upload when all shots/own goals already have this version.")
