"""Build the canonical attempted-pass dataset for xPass model training.

Examples:
  apps/api/.venv/bin/python Data/build_xpass_training_data.py
  apps/api/.venv/bin/python Data/build_xpass_training_data.py --league premier-league --season 2025/2026 --limit-matches 50
"""
...
from xpass_features import build_pass_feature_table, training_frame


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_DB = ROOT / "Data" / "playback90.db"
DEFAULT_OUTPUT = ROOT / "models" / "data" / "xpass_training_passes.parquet"
DEFAULT_REPORT = ROOT / "models" / "data" / "xpass_training_audit.json"
...
def _count_by(df: pd.DataFrame, column: str) -> dict[str, int]:
    if column not in df.columns:
        return {}
    return {str(key): int(value) for key, value in df[column].fillna("unknown").value_counts().sort_index().items()}


def _completion_by(df: pd.DataFrame, column: str) -> dict[str, dict[str, float | int]]:
    if column not in df.columns or df.empty:
        return {}
    out: dict[str, dict[str, float | int]] = {}
    for key, group in df.groupby(column, dropna=False):
        target = pd.to_numeric(group["xpass_completed"], errors="coerce").fillna(0)
        out[str(key)] = {
            "attempts": int(len(group)),
            "completed": int(target.sum()),
            "completion_rate": round(float(target.mean()), 5),
        }
...
def _audit(passes: pd.DataFrame, model_training: pd.DataFrame, duplicate_rows_removed: int = 0) -> dict:
    if passes.empty:
        return {"attempted_passes": 0, "training_passes": 0}

    target = pd.to_numeric(passes.get("xpass_completed", 0), errors="coerce").fillna(0)
    existing_xpass = pd.to_numeric(passes.get("xPass"), errors="coerce") if "xPass" in passes.columns else pd.Series(dtype=float)
    report = {
        "attempted_passes": int(len(passes)),
        "training_passes": int(len(model_training)),
        "completed_passes": int(target.sum()),
        "completion_rate": round(float(target.mean()), 5),
        ...
        "zero_or_tiny_distance": int((pd.to_numeric(passes["pass_distance"], errors="coerce").fillna(0) < 0.1).sum()),
        "duplicate_match_event_ids": int(
            passes.duplicated(subset=[column for column in ("matchId", "eventId") if column in passes.columns]).sum()
        ),
        "duplicate_rows_removed": int(duplicate_rows_removed),
        "existing_xpass_missing": int(existing_xpass.isna().sum()) if not existing_xpass.empty else int(len(passes)),
        "existing_xpass_total": round(float(existing_xpass.fillna(0).sum()), 3) if not existing_xpass.empty else 0.0,
        "pass_distance": _numeric_summary(passes, "pass_distance"),
        "by_league": _count_by(passes, "league"),
        "by_season": _count_by(passes, "season"),
        "by_outcome": _count_by(passes, "outcomeType"),
        ...
        "completion_by_league": _completion_by(passes, "league"),
        "completion_by_pass_type": _completion_by(passes, "pass_type"),
        "completion_by_play_pattern": _completion_by(passes, "play_pattern"),
        "completion_by_pass_direction": _completion_by(passes, "pass_direction"),
    }
    return report
...
def _build_for_match_ids(db_path: Path, match_ids: list[object]) -> pd.DataFrame:
    ...
    for idx, match_id in enumerate(match_ids, start=1):
        ...
        if idx == 1 or idx == total or idx % 25 == 0:
            print(f"[{idx}/{total}] processed match {match_id} -> {len(passes)} attempted pass rows", flush=True)
...
def build_training_data(
    ...
    if league is None and season is None and limit_matches is None:
        ...
        print(f"Building xPass training data across {len(groups)} league/season groups", flush=True)
    ...
    if passes.empty:
        raise RuntimeError("No attempted pass rows were found.")
    ...
    print(f"Wrote {len(passes):,} attempted pass rows to {output}", flush=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build canonical xPass attempted-pass training table.")
